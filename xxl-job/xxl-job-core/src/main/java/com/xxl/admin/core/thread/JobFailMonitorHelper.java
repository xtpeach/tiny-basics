package com.xxl.admin.core.thread;
	...
	public void start(){
		monitorThread = new Thread(new Runnable() {
			...
			public void run() {
				...
				while (!toStop) {
					try {
                        List<String> failLogIds = Lists.newArrayList();
                        if (XxlJobAdminConfig.getAdminConfig().getClusterManager().getAdminInitWaitReady()) {
                            failLogIds = XxlJobAdminConfig.getAdminConfig().getXxlJobLogDao().findFailJobLogIds(1000);
                        }
						...
						if (failLogIds!=null && !failLogIds.isEmpty()) {
                            ...
                            for (int i = startByLocalSortNum; i < failLogIds.size(); i += instanceCount) {
                                failLogIdsDispatched.add(failLogIds.get(i));
                            }

							for (String failLogId: failLogIdsDispatched) {
								...
								XxlJobLogEntity log = XxlJobAdminConfig.getAdminConfig().getXxlJobLogDao().load(failLogId);
								XxlJobInfoEntity info = XxlJobAdminConfig.getAdminConfig().getXxlJobInfoDao().loadById(log.getJobId());
