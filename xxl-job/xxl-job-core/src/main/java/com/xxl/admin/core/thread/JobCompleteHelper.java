...
public class JobCompleteHelper {
			...
			public void run() {
				...
				while (!toStop) {
					try {
                        ...
                        for (int i = startByLocalSortNum; i < losedJobIds.size(); i += instanceCount) {
                            losedJobIdsDispatched.add(losedJobIds.get(i));
                        }
	...
	private ReturnT<String> callback(HandleCallbackParam handleCallbackParam) {
		// valid log item
		XxlJobLogEntity log = XxlJobAdminConfig.getAdminConfig().getXxlJobLogDao().load(handleCallbackParam.getLogId());
		if (log == null) {
