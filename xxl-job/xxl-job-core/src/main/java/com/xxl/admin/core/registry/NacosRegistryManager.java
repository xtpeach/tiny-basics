...
public class NacosRegistryManager {
    ...
    private final static Logger logger = LoggerFactory.getLogger(NacosRegistryManager.class);
    ...
    public void init() {
        ...
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                logger.info("开始周期更新 nacos 执行器信息>>>>>>>>>>>>>>>>>>>>>>>");
                while (!stop) {
                    try {
                ...
                logger.info("结束周期更新 nacos 执行器信息>>>>>>>>>>>>>>>>>>>>>>>");
            }
        });
    ...
    public void updateExecutor() {
        ...
        List<String> serviceNameList = ListUtils.defaultIfNull(discoveryClient.getServices(), Lists.newArrayList())
                .stream().filter(serviceName -> {
                    ...
                    return CollectionUtils.isNotEmpty(serviceInstanceList.stream().map(x -> (NacosServiceInstance) x)
        ...
        if (CollectionUtils.isEmpty(serviceNameList)) {
            // nacos 没有注册任何一个具有执行器能力的微服务，只保留一个 xxl-job-executor-base 默认初始化的执行器，其他历史执行器直接删掉
        ...
        serviceNameList.stream().forEach(serviceName -> {
            ...
            if (CollectionUtils.isNotEmpty(serviceInstanceList)) {
                NacosServiceInstance serviceInstance = (NacosServiceInstance) serviceInstanceList.get(0);
                // 查找已有，更新删除；查找不到，保存
                QueryWrapper<XxlJobGroupEntity> xxlJobGroupEntityQueryWrapper = new QueryWrapper<>();
